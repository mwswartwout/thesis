...
def noisy_odom_remap(odom_msg):
    ...
    if None not in (previous_pose, previous_odom):
        ...
        std_dev_1 = alpha_1 * math.abs(delta_rotation_1) + alpha_2 * delta_translation
        delta_rotation_1_hat = delta_rotation_1 - normal(scale=std_dev_1)

        std_dev_2 = alpha_3 * delta_translation + alpha_4 * (math.abs(delta_rotation_1) + math.abs(delta_rotation_2))
        delta_translation_hat = delta_translation - normal(scale=std_dev_2)

        std_dev_3 = alpha_1 * math.abs(delta_rotation_2) + alpha_2 * delta_translation
        ...
        noisy_odom_yaw = convert_quaternion_to_yaw(previous_pose.pose.pose.orientation) + delta_rotation_1_hat + delta_rotation_2_hat
